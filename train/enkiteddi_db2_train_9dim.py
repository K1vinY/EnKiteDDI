...
def main():
    ...
    print('='*70)
    print('EnKiteDDI DB2 9-dim - Enhanced KiteDDI with SemMedDB Features (9-dim)')
    print('='*70)
    ...
    print('Loading SemMedDB features: ../../data/semmeddb_features_db2_9dim.pkl')
    with open('../../data/semmeddb_features_db2_9dim.pkl', 'rb') as f:
    ...
    for e in range(1, args.n_epoch):
        ...
        if acc_val > best_acc:
            ...
            torch.save(model_bert.state_dict(), 'trained_models/model_enkiteddi_db2_9dim_best.pkl')
            print(f'\n*** Best model saved! Epoch {e}, Val acc: {acc_val:.4f} ***\n')
        
        if e % 10 == 0:
            os.makedirs('./trained_models', exist_ok=True)
            torch.save(model_bert.state_dict(), f'trained_models/model_enkiteddi_db2_9dim_epoch{e}.pkl')
    ...
    os.makedirs('./trained_models', exist_ok=True)
    torch.save(model_bert.state_dict(), 'trained_models/model_enkiteddi_db2_9dim_final.pkl')
    
    print('\n' + '='*70)
    print('EnKiteDDI DB2 9-dim Training Completed!')
    print('='*70)
    print(f'Best Epoch: {best_epoch} | Best Val Accuracy: {best_acc:.4f}')
    print(f'Final model: trained_models/model_enkiteddi_db2_9dim_final.pkl')
    print(f'Best model: trained_models/model_enkiteddi_db2_9dim_best.pkl')
    print('='*70)
